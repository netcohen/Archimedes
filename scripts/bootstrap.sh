...
# ── Wait up to 90s for Archimedes Core to be ready ────────────────────
...
# ── Restart loop ──────────────────────────────────────────────────────
while true; do
    # Clear crash flags in BOTH Snap and regular Chromium pref locations
    for PREF in \\
    ...
    # Brief pause before restart — prevents CPU spin on immediate crash
    sleep 3
done
KIOSKSH
